...
import { useDeclarerLitigeEmplettes } from '@/api/commandes-emplettes';

// Litige déclaré par le livreur lui-même, sans validation par un dispatcher
// (aucun back-office n'existe encore — voir docs/service-emplettes.md).
export function LitigeEmplettesForm({ commandeId }: { commandeId: string }) {
  const [ouvert, setOuvert] = useState(false);
  const [motif, setMotif] = useState('');
  const declarerLitige = useDeclarerLitigeEmplettes();
  ...
  return (
    <div className="space-y-2">
      <Textarea
        placeholder="Article contesté, qualité, écart important..."
        value={motif}
